...
function MainSlider({
    ...
    const initialEagerSlotIdsRef = useRef(
        new Set(virtualPool.map(({ slotId }) => slotId)),
    );
    ...
    const registerForFullImageUpgrade = useCallback((element, notify) => {
        return fullImageObserverControllerRef.current?.register(
            element,
            notify,
        );
    }, []);
    ...
    const createIntroPool = useCallback(() => {
        const initialRange = getVirtualPoolRange(
            starterScrollPosition,
            SLIDE_STEP,
            poolSize,
        );
        const finalRange = getVisibleRange(
    ...
    const commitPool = useCallback((nextPool) => {
        ...
        if (introPoolActiveRef.current) {
            initialEagerSlotIdsRef.current = new Set(
                nextPool.map(({ slotId }) => slotId),
            );
        }
    ...
    useLayoutEffect(() => {
        ...
        commitPool(
            introPoolActiveRef.current
                ? createIntroPool()
                : createSliderRangePool(
                      getVisibleRange(scrollRef.current, LOOP_OVERSCAN_ITEMS),
                  ),
        );
    ...
    const syncVirtualPoolForScroll = useCallback(
        (scroll) => {
            ...
            if (isSliderRangeCovered(virtualPoolRef.current, visibleRange))
                return;
            commitPool(
                createSliderRangePool(
                    getVisibleRange(scroll, LOOP_OVERSCAN_ITEMS),
                ),
            );
        },
        ...
        (timestamp) => {
            ...
            if (previousTimestamp !== null) {
                if (sliderProfileRef.current?.loopGaps.length < 1800) {
                    sliderProfileRef.current.loopGaps.push(
                ...
                const nextSpeed =
                    Math.abs(speedDifference) <= maximumSpeedChange
                        ? targetSpeed
                        : currentSpeed +
                          Math.sign(speedDifference) * maximumSpeedChange;

                autoScrollCurrentSpeedRef.current = nextSpeed;

                if (nextSpeed !== 0) {
                    setScrollValue(
                        scrollRef.current + (nextSpeed * elapsed) / 1000,
                    );
                }
            }
    ...
    const runAnimation = useCallback(() => {
        ...
        const continuousSpeedBlend =
            scrollDistance === 0
                ...
                : Math.min(
                      Math.max(
                          (autoScrollSpeed * durationSeconds) / scrollDistance,
                          0,
                      ),
                      1,
                  );
    ...
    const dismissLoaderAndScheduleAnimation = useCallback(() => {
        if (
            ...
            loaderDismissAnimationFrameRef.current
        ) {
            return;
        }
    ...
    const onInitialPreviewSettled = useCallback(
        (slotId) => {
            if (
                initialLoadingDoneRef.current ||
                animationStartedRef.current ||
                animationEndedRef.current
            )
                return;
    ...
    useEffect(() => {
        return () => {
            ...
            if (titlePointerAnimationFrameRef.current) {
                window.cancelAnimationFrame(
                    titlePointerAnimationFrameRef.current,
                );
            }
            if (reopenAnimationTimeoutRef.current) {
                window.cancelAnimationFrame(reopenAnimationTimeoutRef.current);
    ...
    return (
        <div
            ...
            onPointerDown={!isLeaving ? handlePointerDown : undefined}
            onPointerMove={!isLeaving ? handlePointerMove : undefined}
            onPointerUp={!isLeaving ? handlePointerUp : undefined}
            onPointerCancel={!isLeaving ? handlePointerCancel : undefined}
            style={{
                touchAction: !isLeaving ? "none" : "auto",
                overflow: "hidden",
                visibility: isActive ? "visible" : "hidden",
                pointerEvents:
                    isActive && !isHidden && animationEnded && !isLeaving
                        ? "auto"
                        : "none",
            }}
        >
            ...
            <div
                className={`${styles.sliderScene} ${isEntering ? styles.sliderSceneEntering : ""} ${!animationEnded ? styles.sliderSceneIntro : ""} ${isLeaving ? styles.sliderSceneLeaving : ""} ${isHidden ? styles.sliderSceneHidden : ""}`}
                style={{
            ...
            <button
                ...
                style={{
                    opacity: isActive && animationEnded && !isLeaving ? 1 : 0,
                    pointerEvents:
                        isActive && animationEnded && !isLeaving
                            ? "auto"
                            : "none",
                }}
